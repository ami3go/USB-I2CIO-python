...
class FakeDevice:
    ...
    def write_no_stop(self, address, data):
        self.calls.append(("write_no_stop", address, bytes(data)))
        return len(data)
...
def make_adapter(allow_stop_fallback=True, use_no_stop_for_generic_read=False):
    i2c = DevasysBlinkaI2C.__new__(DevasysBlinkaI2C)
    i2c._dev = FakeDevice()
    i2c.frequency = None
    i2c._locked = False
    i2c._allow_stop_fallback = allow_stop_fallback
    i2c._use_no_stop_for_generic_read = use_no_stop_for_generic_read
...
def test_long_write_then_read_no_stop_path():
    i2c = make_adapter(allow_stop_fallback=False, use_no_stop_for_generic_read=True)
    buf = bytearray(2)

    i2c.writeto_then_readfrom(0x20, bytes([1, 2, 3]), buf)

    assert ("write_no_stop", 0x20, bytes([1, 2, 3])) in i2c._dev.calls
    assert i2c._dev.calls[-1] == ("read", 0x20, 2)
