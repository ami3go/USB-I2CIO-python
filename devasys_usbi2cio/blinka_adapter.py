...
class DevasysBlinkaI2C:
    ...
    def writeto(
        ...
        *,
        ...
        """Write bytes to an I2C device using a CircuitPython-compatible signature."""
        if end is None:
            end = len(buffer)
        ...
        try:
            if stop:
                ...
                # Some Adafruit probing code uses zero-length writes. The
                # DeVaSys raw write wrapper does not allow zero-length writes,
                # so use a one-byte read probe instead.
                if not data:
            ...
            # Store transaction for a following readfrom_into(). This is the
            # usual CircuitPython/Blinka stop=False pattern.
    ...
    def _write_then_read_compat(self, address: int, out_data: bytes, read_count: int) -> bytes:
        ...
        For longer output data, the adapter first tries a no-stop write
        transaction if enabled, otherwise it falls back to write-with-STOP
        followed by read when allow_stop_fallback=True.
        """
        if read_count <= 0:
            return bytes()
        ...
        if getattr(self, "_use_no_stop_for_generic_read", True):
            self._dev.write_no_stop(address, out_data)
            return self._dev.read(address, read_count)
